...
class TaiwanProtocolTests(unittest.TestCase):
    ...
    def test_process_wide_api_interval_waits_for_remaining_gap(self):
        api_module = importlib.import_module("autopcr.core.apiclient")
        previous = api_module._last_api_call_at
        sleeper = AsyncMock()
        try:
            api_module._last_api_call_at = time.monotonic() - 0.2
            with patch.object(
                api_module, "API_CALL_INTERVAL", 0.5
            ), patch.object(api_module, "sleep", sleeper):
                self.loop.run_until_complete(
                    api_module._wait_for_api_interval()
                )
        finally:
            api_module._last_api_call_at = previous

        sleeper.assert_awaited_once()
        self.assertGreater(sleeper.await_args.args[0], 0.25)
        self.assertLessEqual(sleeper.await_args.args[0], 0.3)
    ...
    def test_unknown_server_campaign_ids_are_ignored(self):
        ...
        self.assertEqual(4000, campaign_times)
    ...
    def test_audited_tw_modules_are_allowlisted_without_protocol_exceptions(self):
        registered = set(ModuleListManager.name_to_modules)
        protocol_exceptions = {"monthly_gacha"}

        self.assertLessEqual(
            registered - protocol_exceptions,
            TW_SUPPORTED_MODULES,
        )
        self.assertTrue(protocol_exceptions.isdisjoint(TW_SUPPORTED_MODULES))
        self.assertIn("labyrinth_start_reroll", TW_SUPPORTED_MODULES)
    ...
    def test_tw_response_extensions_are_preserved(self):
...
class TaiwanCredentialTests(unittest.TestCase):
    def test_composite_credentials_are_normalized_and_use_merged_server(self):
        ...
        self.assertEqual(["https://api5-pc.so-net.tw/"], sdk.apiroots)
