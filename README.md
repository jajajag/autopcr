...
程序当前默认使用 `APP_VERSION=5.8.1`。Android 5.7.0 包内已核验的运行时为 `Unity 6000.0.58f2`、`libcurl 8.10.1-DEV`，HTTP 资源版本兜底值为 `00500030`；这些值均可通过下方环境变量独立覆盖，无需修改国服配置。HTTP `RES-VER` 与 master database 版本是两个独立的版本号，不能相互替代。
...
台服账号开放当前已注册的模块，包括旅行、额外装备、商队、深域、属性强化、黎明界随机开局及相关规划工具。究极炼成的台服 `exec_type` 和赛季通行证的 `battlepass/*` 端点使用区域专用请求体，国服仍保持原协议分支。目前仅隐藏已知协议不兼容的 `monthly_gacha`；活动和客户端更新仍可能带来协议变化。
...
| AUTOPCR_API_CALL_INTERVAL | 同一账号收到 API 响应后到下一次调用的最小间隔（秒）；设为 0 可关闭 | 0.3 |
| AUTOPCR_STAMINA_API_CALL_INTERVAL | 同一账号发起消耗体力 API 前的最小间隔（秒），不会低于普通 API 间隔 | 1.0 |
